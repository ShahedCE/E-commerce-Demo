...
function drawImageProp(ctx, img, x, y, w, h, offsetX, offsetY) {
  ...
  const isDesktop = (w >= 980) || (w > h);
  
  let r;
  if (isDesktop) {
    // Landscape (Laptop/Desktop/Tablet) or Desktop site simulated width:
    // Make it completely full-bleed cover, filling the entire screen track with zero empty borders!
    r = Math.max(w / iw, h / ih);
  } else {
    // Standard Portrait (Mobile) - scale image to fit 100% of the screen width
    // This prevents any side-cropping and ensures the sunglasses are fully visible.
    r = w / iw;
  }
